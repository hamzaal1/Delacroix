...
export async function POST(request) {
    try {
        ...
        await trasnport.sendMail({
            ...
            subject: `Recrutement sur Filière ${subject}, Mr/Mme ${name}`,
            html: `
                <html>
                    <head>
                        <style>
                    ...
                    <body>
                        <div class="container">
                            <h1>Recrutement Form Submission</h1>
                            <ul>
            ...
            attachments: [
                {
                    filename: `${name}.pdf`,
                    content: Buffer.from(fileBuffer), // Use the file content as Buffer
                    contentType: file.type
                },
            ],
        });

        return NextResponse.json({ message: 'Email sent successfully!', status: "ok" });
    } catch (error) {
        return NextResponse.json({ error: `Failed to send email with attachment.${error}` });
    }
}
